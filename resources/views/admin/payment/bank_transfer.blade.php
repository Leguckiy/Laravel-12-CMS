...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="form-payment" action="{{ route('admin.payment.update', $method['code']) }}" method="POST">
                @csrf
                @method('PUT')
                <x-admin.textarea-field-multilang
                    name="instructions"
                    :label="__('admin.payment_bank_transfer_instructions')"
                    :placeholder="__('admin.payment_bank_transfer_instructions')"
                    :value="$instructionsTranslations ?? []"
                    :languages="$languages"
